...
export default function ConsultingList() {
  ...
  return (
    <StMyPageBodyWrap>
      ...
      <ul>
        <li onClick={onClickListAll} style={{ fontWeight: listState === 0 ? 'bolder' : 'normal' }}>
          전체
        </li>
        <li onClick={onClickListWait} style={{ fontWeight: listState === 1 ? 'bolder' : 'normal' }}>
          대기중
        </li>
        <li onClick={onClickListComplete} style={{ fontWeight: listState === 2 ? 'bolder' : 'normal' }}>
          완료된 상담
        </li>
      </ul>
