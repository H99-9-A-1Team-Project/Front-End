...
export default function NewFootStepSecurityState() {
  const [securityPathState, setSecurityPathState] = useRecoilState(nfsrPath);

  const onSunStateChange = () => {
    if (securityPathState.security === false) setSecurityPathState({ ...securityPathState, security: true });
